...
export async function POST(request: Request) {
  try {
    ...
    const user = await db.user.create({ data: { name, email, passwordHash: await hashPassword(password), memberships: { create: { role: 'OWNER', organization: { create: { name: `${name}'s workspace`, } } } } } });
