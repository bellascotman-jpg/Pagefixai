...
export async function POST(request: Request) {
  ...
  let payload: any;
  try { payload = await request.json(); } catch { return NextResponse.json({ error: 'VALIDATION_ERROR' }, { status: 400 }); }
  ...
  await db.webhookEvent.upsert({ where: { provider_providerEventId: { provider: 'flutterwave', providerEventId: eventId } }, create: { provider: 'flutterwave', providerEventId: eventId, payload }, update: { payload } });
  try {
    ...
    await db.$transaction(async (tx) => {
      const payment = await tx.payment.upsert({ where: { providerReference: txRef }, create: { userId, provider: 'flutterwave', providerReference: txRef, amount: verified.amount, currency: verified.currency, status: 'successful', planId, metadata: payload }, update: { status: 'successful', metadata: payload } });
      ...
      const subscription = await tx.subscription.upsert({ where: { id: `flw_${txRef}` }, create: { id: `flw_${txRef}`, userId, planId, status, provider: 'flutterwave', providerReference: txRef, expiresAt }, update: { status, expiresAt } });
      const features = ['AUDIT', 'REPORTS', 'FIX_CENTER', ...(planId.includes('growth') || planId.includes('agency') || planId === 'founding_lifetime' ? ['COMPETITOR', 'MESSAGE_MATCH', 'COPY_STUDIO'] : []), ...(planId.includes('agency') ? ['AGENCY_WORKSPACE'] : [])];
      for (const feature of features) await tx.entitlement.upsert({ where: { userId_feature: { userId, feature } }, create: { userId, feature, source: subscription.id, active: true, expiresAt }, update: { active: true, source: subscription.id, expiresAt } });
      await tx.notification.create({ data: { userId, type: 'PAYMENT_SUCCESS', title: 'Payment confirmed', body: `${plan.name} access is now active.` } });
      void payment;
    });
    await db.webhookEvent.update({ where: { provider_providerEventId: { provider: 'flutterwave', providerEventId: eventId } }, data: { processedAt: new Date() } });
